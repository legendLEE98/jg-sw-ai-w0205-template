...
def tourism(visited:list, curzone:int, cost:int):
    if allVisited(visited):
        lastCost = int(coord[curzone][startZone])
        if lastCost != 0:
            costList.append(cost + lastCost)
        return

    for i in range(0, len(visited)):
        # 아직 투어하지 못한 곳을 발견했다면
        # 우선 어디에서 어디로 가는지 추적해야 함.
        if int(coord[curzone][i]) == 0:
            continue
        if visited[i] == False:
            visited[i] = True
            tourism(visited, i, cost + int(coord[curzone][i]))
            visited[i] = False

def allVisited(visited:list):
